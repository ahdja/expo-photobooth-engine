...
__export(index_exports, {
  PhotoboothFrameGenerator: () => PhotoboothFrameGenerator
});
...
var PhotoboothFrameGenerator = class {
  ...
  async loadImage(source) {
    if (source instanceof Uint8Array) {
      return import_react_native_skia.Skia.Image.MakeImageFromEncoded(import_react_native_skia.Skia.Data.fromBytes(source));
    }
    if (typeof source === "string") {
      if (source.startsWith("data:")) {
        const base64Data = source.split(",")[1];
        return import_react_native_skia.Skia.Image.MakeImageFromEncoded(import_react_native_skia.Skia.Data.fromBase64(base64Data));
      }
      try {
  ...
  drawCover(canvas, img, slot) {
    ...
    const rect = import_react_native_skia.Skia.XYWHRect(-targetW / 2, -targetH / 2, targetW, targetH);
    const srcRect = import_react_native_skia.Skia.XYWHRect(sx, sy, sw, sh);
    const paint = import_react_native_skia.Skia.Paint();
